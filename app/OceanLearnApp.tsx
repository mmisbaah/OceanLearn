...
function LessonLibrary({student,progress,award}:{student:Student;progress:Progress;award:(id:string,n?:number)=>void}){const grade=effectiveGrade(student);const[lesson,setLesson]=useState<number|null>(null);const finish=()=>{if(lesson===null)return;award(`lesson-${grade}-${lesson}`,5);setLesson(lesson<LESSONS[grade].length-1?lesson+1:null)};return <section><PageTitle eyebrow={`GRADE ${grade} • COMPLETE CURRICULUM`} title="Learning Lagoon" text="Sixteen curriculum lessons, each with five guided learning steps."/>{lesson===null?<div className="curriculum-grid">{LESSONS[grade].map((name,i)=>{const done=progress.completed.includes(`lesson-${grade}-${i}`);return <button className="curriculum-card" key={name} onClick={()=>setLesson(i)}><span>{done?"✓":i+1}</span><div><small>{i<8?"TERM 1":"TERM 2"} • {SKILLS[i%5]}</small><strong>{name}</strong><em>5 sub-lessons</em></div></button>})}</div>:<LessonPlayer grade={grade} index={lesson} onBack={()=>setLesson(null)} onComplete={finish}/>}</section>}

function LessonPlayer({grade,index,onBack,onComplete}:{grade:number;index:number;onBack:()=>void;onComplete:()=>void}){
  const[step,setStep]=useState(0); const[answerOpen,setAnswerOpen]=useState(false); const topic=LESSONS[grade][index]; const detail=lessonDetail(topic,grade,step);
  useEffect(()=>{scrollTop()},[index,step]);
  const move=(next:number)=>{setStep(next);setAnswerOpen(false)};
  return <article className="lesson-player"><button className="back-button" onClick={onBack}>← All lessons</button><div className="lesson-player-head"><img src={`/assets/generated/teach-${MASCOTS[index%5]}.png`} alt="Lesson guide in a teaching pose"/><div><span className="eyebrow">LESSON {index+1} • SUB-LESSON {step+1} OF 5</span><h1>{topic}</h1><p>{SKILLS[step]}</p></div></div><div className="step-dots">{SKILLS.map((s,i)=><button key={s} className={i===step?"current":i<step?"finished":""} onClick={()=>move(i)}><span>{i<step?"✓":i+1}</span><small>{s}</small></button>)}</div><div className="lesson-explanation"><span className="pill">{SKILLS[step]}</span><h2>{detail.title}</h2>{detail.paragraphs.map((p,i)=><p key={i}>{p}</p>)}<div className="lesson-examples"><strong>Try these steps</strong><ol>{detail.examples.map(x=><li key={x}>{x}</li>)}</ol></div><div className="guided-practice"><strong>Guided practice</strong><p>{detail.practice}</p><button onClick={()=>setAnswerOpen(!answerOpen)}>{answerOpen?"Hide learning check":"Show learning check"}</button>{answerOpen&&<p className="practice-answer">💡 {detail.answer}</p>}</div></div><div className="lesson-controls"><button onClick={()=>move(Math.max(0,step-1))} disabled={step===0}>← Previous</button>{step<4?<button className="primary-button compact" onClick={()=>move(step+1)}>Next sub-lesson →</button>:<button className="primary-button compact" onClick={onComplete}>Finish & go to next lesson →</button>}</div></article>}

function QuizLibrary({student,progress,award}:{student:Student;progress:Progress;award:(id:string,n?:number)=>void}){const grade=effectiveGrade(student);const[set,setSet]=useState<number|null>(null);return <section><PageTitle eyebrow={`${student.difficulty.toUpperCase()} • GRADE ${grade} SKILLS`} title="Quiz Cove" text="Twenty quiz sets with five questions in every set."/>{set===null?<div className="level-grid">{Array.from({length:20},(_,i)=><button key={i} className={progress.completed.includes(`quiz-${grade}-${i}`)?"level done":"level"} onClick={()=>setSet(i)}><strong>{i+1}</strong><span>Quiz set</span><small>{i<7?"Easy":i<14?"Medium":"Hard"} • 5 questions</small></button>)}</div>:<QuizPlayer grade={grade} set={set} onBack={()=>setSet(null)} onComplete={()=>award(`quiz-${grade}-${set}`,8)}/>}</section>}

function QuizPlayer({grade,set,onBack,onComplete}:{grade:number;set:number;onBack:()=>void;onComplete:()=>void}){
  const[pos,setPos]=useState(0);const[choice,setChoice]=useState<number|null>(null);const[checked,setChecked]=useState(false);const[score,setScore]=useState(0);const q=makeQuestion(grade,set,pos);
...
function GameLibrary({student,progress,award}:{student:Student;progress:Progress;award:(id:string,n?:number)=>void}){
  const[game,setGame]=useState<number|null>(null);const[level,setLevel]=useState<number|null>(null);const grade=effectiveGrade(student);
  useEffect(()=>{scrollTop()},[game,level]);
  if(game!==null&&level!==null)return <GamePlayer key={`${game}-${level}`} game={game} level={level} grade={grade} onBack={()=>setLevel(null)} onWin={()=>award(`game-${game}-${level}`,8)} onNext={()=>setLevel(level<19?level+1:null)}/>;
  if(game!==null)return <section><button className="back-button" onClick={()=>setGame(null)}>← All games</button><PageTitle eyebrow="20 PLAYABLE LEVELS" title={GAME_META[game].name} text={GAME_META[game].description}/><div className="level-grid">{Array.from({length:20},(_,i)=><button key={i} className={progress.completed.includes(`game-${game}-${i}`)?"level done":"level"} onClick={()=>setLevel(i)}><strong>{i+1}</strong><span>Level</span><small>{i<7?"Easy":i<14?"Medium":"Hard"} • 5 questions</small></button>)}</div></section>;
...
function GamePlayer({game,level,grade,onBack,onWin,onNext}:{game:number;level:number;grade:number;onBack:()=>void;onWin:()=>void;onNext:()=>void}){
  const[pos,setPos]=useState(0);const[choice,setChoice]=useState<number|null>(null);const[checked,setChecked]=useState(false);const[score,setScore]=useState(0);const q=makeGameQuestion(game,grade,level,pos);
  useEffect(()=>{scrollTop()},[level,pos]);
  const check=()=>{if(choice===null)return;setChecked(true);if(choice===q.answer)setScore(s=>s+1)};const advance=()=>{setPos(p=>p+1);setChoice(null);setChecked(false)};
  return <section className="game-stage"><button className="back-button" onClick={onBack}>← Levels</button><div className="game-stage-head"><div><span className="eyebrow">{GAME_META[game].name.toUpperCase()} • {level<7?"EASY":level<14?"MEDIUM":"HARD"}</span><h1>Level {level+1}</h1><p>Question {pos+1} of 5 • Grade {grade}</p></div></div><article className="game-question"><div className="level-meter"><span style={{width:`${(pos+1)*20}%`}}/></div><h2>{q.prompt}</h2><div className="options">{q.options.map((o,i)=><button key={o} className={`${choice===i?"chosen":""} ${checked&&i===q.answer?"correct":""} ${checked&&choice===i&&i!==q.answer?"wrong":""}`} disabled={checked} onClick={()=>setChoice(i)}><span>{["🐚","🐠","⭐"][i]}</span>{o}</button>)}</div>{checked&&<div className={choice===q.answer?"feedback success":"feedback try"}><strong>{choice===q.answer?"Great move!":"Good try—use the clue."}</strong><p>{q.explanation}</p></div>}{pos===4&&checked?<div className="game-finish"><strong>Level complete! Score {score}/5</strong><button className="primary-button compact" onClick={()=>{onWin();onNext()}}>{level<19?"Start next level →":"Back to levels"}</button></div>:<button className="primary-button compact" disabled={choice===null} onClick={checked?advance:check}>{checked?"Next question →":"Check my move"}</button>}</article></section>
}
