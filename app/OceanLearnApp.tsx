...
function speakDictation(text:string,stage:number){
 if(typeof window==="undefined"||!("speechSynthesis" in window))return;
 const profile=DICTATION_SPEECH[Math.min(6,Math.max(0,stage))];window.speechSynthesis.cancel();const voice=new SpeechSynthesisUtterance(text.trim());voice.lang="en-GB";voice.rate=profile.rate;voice.pitch=profile.pitch;voice.volume=1;window.speechSynthesis.speak(voice);
}
...
function LessonPlayer({stage,index,onBack,onComplete}:{stage:number;index:number;onBack:()=>void;onComplete:()=>void}){
  ...
  const speech=[detail.title,...contextualParagraphs,...story,...detail.examples,detail.practice].join(". ");
  return <article className="lesson-player"><button className="back-button" onClick={onBack}>← All lessons</button><div className="lesson-player-head"><img src={`/assets/generated/teach-${MASCOTS[index%5]}.png`} alt="Lesson guide in a teaching pose"/><div><span className="eyebrow">LESSON {index+1} • SUB-LESSON {step+1} OF 5</span><h1>{topic}</h1><p>{foundation?`TIER ${foundation.tier} • ALPHABET FOUNDATIONS`:SKILLS[step]}</p></div></div><div className="step-dots">{SKILLS.map((s,i)=><button key={s} className={i===step?"current":i<step?"finished":""} onClick={()=>move(i)}><span>{i<step?"✓":i+1}</span><small>{s}</small></button>)}</div><div className={`lesson-explanation ${stage===0?"foundation-step":""}`}><div className="self-guide" aria-label="Learning steps"><span>1 👀 Look</span><span>2 🔊 Hear</span><span>3 👄 Say</span><span>4 👆 Do</span><span>5 ⭐ Check</span></div><button className="hear-button" onClick={()=>speak(speech)}>🔊 Hear this step</button><span className="pill">{SKILLS[step]}</span><h2>{detail.title}</h2>{detail.paragraphs.map((p,i)=><p key={i}>{p}</p>)}{foundation&&<section className="read-learn"><span className="eyebrow">📖 READ & LEARN</span><h3>Read with me</h3>{foundation.story.map((line,i)=><p key={line}><span aria-hidden="true">{pictureFor(line,stage,i)}</span>{line}</p>)}<button onClick={()=>speak(foundation.story.join(" "))}>🔊 Hear the story</button></section>}{foundation?<InlinePractice key={`${index}-${step}`} activity={foundation.activities[step]} onSolved={()=>setPracticeDone(true)}/>:<><div className={`lesson-examples picture-examples stage-${stage}`}><strong>{stage===0?"Look and do":"Picture examples"}</strong><div>{detail.examples.map((x,i)=><article key={x}><span aria-hidden="true">{pictureFor(`${topic} ${x}`,stage,i)}</span><p>{x}</p></article>)}</div></div><div className="guided-practice"><strong>{stage===0?"Your turn":"Guided practice"}</strong><p>{detail.practice}</p><button onClick={()=>setAnswerOpen(!answerOpen)}>{answerOpen?"Hide answer":"Check my try"}</button>{answerOpen&&<p className="practice-answer">💡 {detail.answer}</p>}</div></>}</div><div className="lesson-controls"><button onClick={()=>move(Math.max(0,step-1))} disabled={step===0}>← Previous</button>{step<4?<button className="primary-button compact" disabled={!!foundation&&!practiceDone} onClick={()=>move(step+1)}>{foundation&&!practiceDone?"Solve the activity first":"Next sub-lesson →"}</button>:<button className="primary-button compact" disabled={!!foundation&&!practiceDone} onClick={onComplete}>{foundation&&!practiceDone?"Solve the activity first":"Finish & go to next lesson →"}</button>}</div></article>}
...
function QuizPlayer({grade,stage,set,onBack,onComplete}:{grade:number;stage:number;set:number;onBack:()=>void;onComplete:()=>void}){
 ...
 return <article className="quiz-player"><button className="back-button" onClick={onBack}>← Quiz sets</button><div className="quiz-status"><span>Set {set+1}</span><strong>Question {pos+1} of 5</strong><span>Score {score}</span></div><span className="question-type">{label}</span>{q.passage&&<section className="quiz-passage"><strong>Read this island story</strong><p>{q.passage}</p></section>}{q.type==="dictation"?<button className="dictation-player" onClick={()=>speakDictation(q.audioText??"",stage)}>🔊 Hear slowly</button>:<div className={`question-picture stage-${stage}`}><span>{pictureFor(q.passage??q.prompt,stage,pos)}</span><small>{q.type==="reading-comprehension"?"Story picture":"Look at the picture"}</small></div>}<QuestionPrompt key={q.token} prompt={q.prompt}/>{hasOptions?<div className="options picture-options">{q.options.map((o,i)=><button key={o} disabled={checked} className={`${choice===i?"chosen":""} ${checked&&i===q.answer?"correct":""} ${checked&&choice===i&&i!==q.answer?"wrong":""}`} onClick={()=>setChoice(i)}><span>{String.fromCharCode(65+i)}</span><i aria-hidden="true">{pictureFor(o,stage,i)}</i><b>{cleanOptionText(o)}</b></button>)}</div>:<label className="typed-answer"><span>{q.type==="dictation"?"Type what you hear":"Complete the blank"}</span><input value={typed} disabled={checked} onChange={e=>setTyped(e.target.value)} placeholder={q.placeholder} autoCapitalize="none"/></label>}{checked&&<div className={correct?"feedback success":"feedback try"}><strong>{correct?"Fin-tastic!":"Good try!"}</strong><p>{q.explanation}</p></div>}<button className="primary-button compact" disabled={!ready} onClick={checked?next:check}>{checked?(pos===4?"Finish quiz":"Next question →"):"Check answer"}</button></article>
}
...
function GamePlayer({game,level,stage,onBack,onWin,onNext}:{game:number;level:number;stage:number;onBack:()=>void;onWin:()=>void;onNext:()=>void}){
  ...
  return <section className={`game-stage play-mode mode-${game}`}><button className="back-button" onClick={onBack}>← Levels</button><div className="game-stage-head"><div><span className="eyebrow">{GAME_META[game].name.toUpperCase()} • {level<7?"FOUNDATION":level<14?"APPLICATION":"CHALLENGE"}</span><h1>Level {level+1}</h1><p>{STAGES[stage].label} • Mission {pos+1} of 5</p></div></div><article className="game-question"><div className="game-hud"><span>❤️❤️❤️</span><strong>🔥 Combo {streak}</strong><span>⭐ {score}</span></div><div className="level-meter"><span style={{width:`${(pos+1)*20}%`}}/></div><div className="game-scene" aria-hidden="true"><span>{icons[0]}</span><b>{pictureFor(q.prompt,stage,pos)}</b><i>{icons[2]}</i></div><QuestionPrompt key={q.token} prompt={q.prompt}/><p className="game-instruction">Tap one picture. It moves by itself!</p><div className="game-options">{q.options.map((o,i)=><button key={o} className={`${choice===i?"chosen":""} ${checked&&i===q.answer?"correct":""} ${checked&&choice===i&&i!==q.answer?"wrong":""}`} disabled={checked} onClick={()=>choose(i)}><span>{pictureFor(o,stage,i)}</span><b>{o}</b></button>)}</div>{checked&&<div className={choice===q.answer?"feedback success":"feedback try"}><strong>{choice===q.answer?"Yes! Great move!":"Oops! Try the next one."}</strong><p>{q.explanation}</p></div>}{pos<4&&<p className="auto-next-note" aria-live="polite">{checked?"Next puzzle…":"Tap an answer."}</p>}{pos===4&&checked&&(score>=3?<div className="game-finish"><strong>Level passed! Score {score}/5</strong><button className="primary-button compact" onClick={()=>{onWin();onNext()}}>{level<19?"Sail to next level →":"Back to levels"}</button></div>:<div className="game-finish fail"><strong>Almost! Score {score}/5</strong><p>Find 3 treasures to pass.</p><button className="primary-button compact" onClick={retry}>Try this level again ↻</button></div>)}</article></section>
}
