...
function LessonPlayer({stage,index,onBack,onComplete}:{stage:number;index:number;onBack:()=>void;onComplete:()=>void}){
  const grade=STAGES[stage].grade;const[step,setStep]=useState(0); const[answerOpen,setAnswerOpen]=useState(false); const topic=lessonsForStage(stage)[index]; const detail=lessonDetail(topic,grade,step);
...
function GamePlayer({game,level,stage,onBack,onWin,onNext}:{game:number;level:number;stage:number;onBack:()=>void;onWin:()=>void;onNext:()=>void}){
  const grade=STAGES[stage].grade;const[pos,setPos]=useState(0);const[choice,setChoice]=useState<number|null>(null);const[checked,setChecked]=useState(false);const[score,setScore]=useState(0);const[streak,setStreak]=useState(0);const[salt]=useState(()=>Math.floor(Math.random()*2**31));const q=gameQuestion(game,stage,level,pos,salt);
  useEffect(()=>{scrollTop()},[level,pos]);
  const check=()=>{if(choice===null)return;setChecked(true);if(choice===q.answer){setScore(s=>s+1);setStreak(s=>s+1)}else setStreak(0)};const advance=()=>{setPos(p=>p+1);setChoice(null);setChecked(false)};
  const icons=[["🐚","🫧","💎"],["🔤","🐝","✨"],["🏁","🐢","⚡"],["🧩","📖","🌟"],["🛠️","🦀","🛡️"]][game];
  return <section className={`game-stage play-mode mode-${game}`}><button className="back-button" onClick={onBack}>← Levels</button><div className="game-stage-head"><div><span className="eyebrow">{GAME_META[game].name.toUpperCase()} • {level<7?"FOUNDATION":level<14?"APPLICATION":"CHALLENGE"}</span><h1>Level {level+1}</h1><p>{STAGES[stage].label} • Mission {pos+1} of 5</p></div></div><article className="game-question"><div className="game-hud"><span>❤️❤️❤️</span><strong>🔥 Combo {streak}</strong><span>⭐ {score}</span></div><div className="level-meter"><span style={{width:`${(pos+1)*20}%`}}/></div><div className="game-scene" aria-hidden="true"><span>{icons[0]}</span><b>{icons[1]}</b><i>{icons[2]}</i></div><h2>{q.prompt}</h2><p className="game-instruction">Tap a floating treasure to make your move!</p><div className="game-options">{q.options.map((o,i)=><button key={o} className={`${choice===i?"chosen":""} ${checked&&i===q.answer?"correct":""} ${checked&&choice===i&&i!==q.answer?"wrong":""}`} disabled={checked} onClick={()=>setChoice(i)}><span>{["🫧","🐚","💎"][i]}</span><b>{o}</b></button>)}</div>{checked&&<div className={choice===q.answer?"feedback success":"feedback try"}><strong>{choice===q.answer?"Splash-tastic treasure!":"Oops—a cheeky crab moved it!"}</strong><p>{q.explanation}</p></div>}{pos===4&&checked?<div className="game-finish"><strong>Mission complete! Score {score}/5</strong><button className="primary-button compact" onClick={()=>{onWin();onNext()}}>{level<19?"Sail to next level →":"Back to levels"}</button></div>:<button className="primary-button compact" disabled={choice===null} onClick={checked?advance:check}>{checked?"Next mission →":"Launch my move!"}</button>}</article></section>
}
