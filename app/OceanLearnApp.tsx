...
function VocabularyDeck({stage,lesson,step,lessonCount}:{stage:number;lesson:number;step:number;lessonCount:number}){
 ...
 return <section className={`word-deck word-deck-stage-${stage}`} aria-labelledby="word-deck-title"><header><span aria-hidden="true">{stage===0?"👀":"🧠"}</span><div><strong id="word-deck-title">{stage===0?"Look, tap and say":"Focus words"}</strong><small>{stage===0?"Tap the big picture. Hear the word. Say it.":"Tap a picture to hear its word."}</small></div></header><div>{words.map(word=>{const visual=wordVisual(word,knownMeanings.get(word.toLowerCase()));return <button type="button" key={word} onClick={()=>speak(word)} aria-label={`Hear the word ${word}`}><span className="word-picture" aria-hidden="true">{visual.icon}</span><strong>{word}</strong><small>{visual.picture}</small>{stage>0&&<p><b>Meaning:</b> {visual.meaning}</p>}<em>🔊 Tap to hear</em></button>})}</div></section>;
}
