...
function LessonPlayer({stage,index,onBack,onComplete}:{stage:number;index:number;onBack:()=>void;onComplete:()=>void}){
  const grade=STAGES[stage].grade;const[step,setStep]=useState(0); const[answerOpen,setAnswerOpen]=useState(false);const[practiceDone,setPracticeDone]=useState(false); const topic=lessonsForStage(stage)[index];const foundation=stage===0?GRADE1_EASY_PATH.firstFive[index]:undefined; const detail=foundation?{title:step===1?"Read & Learn":"Learn the letters",paragraphs:[foundation.outcome],examples:[],practice:"",answer:""}:stage===0?foundationLessonDetail(index,step):lessonDetail(topic,grade,step);
  ...
  const speech=[detail.title,...detail.paragraphs,...detail.examples,detail.practice].join(". ");
