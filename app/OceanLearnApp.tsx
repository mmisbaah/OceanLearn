...
export default function OceanLearnApp(){
  ...
  return <div className="app-shell">
    <header className="top-dock"><button className="brand" onClick={()=>setSection("home")} aria-label="OceanLearn home"><img src="/assets/generated/dolphin.png" alt=""/><span>Ocean<span>Learn</span></span></button><div className="top-treasures"><span>⭐ <strong>{progress.stars}</strong></span><span>🏅 <strong>{progress.badges.length}</strong></span></div><div className="student-chip"><img src={avatarPath(student.avatar)} alt="Selected student avatar"/><div><strong>Hi, {student.name}</strong><small className="student-level" style={{display:"block"}}>Grade {student.grade} • {student.difficulty}</small></div></div><div className="top-actions"><button onClick={()=>setResetOpen(true)} aria-label="Reset progress"><b aria-hidden="true">🫧</b><span>Reset</span></button><button onClick={logout} aria-label="Logout"><b aria-hidden="true">🚪</b><span>Logout</span></button></div></header>
