...
function QuizPlayer({stage,set,onBack,onComplete}:{stage:number;set:number;onBack:()=>void;onComplete:()=>void}){
  ...
  return <article className="quiz-player"><button className="back-button" onClick={onBack}>← Quiz sets</button><div className="quiz-status"><span>Set {set+1}</span><strong>Question {pos+1} of 5</strong><span>Score {score}</span></div><div className={`question-picture stage-${stage}`}><span>{pictureFor(q.prompt,stage,pos)}</span><small>{stage<=1?"Look at the picture":"Use the picture clue"}</small></div><h2>{q.prompt}</h2><div className="options picture-options">{q.options.map((o,i)=><button key={o} disabled={checked} className={`${choice===i?"chosen":""} ${checked&&i===q.answer?"correct":""} ${checked&&choice===i&&i!==q.answer?"wrong":""}`} onClick={()=>setChoice(i)}><span>{String.fromCharCode(65+i)}</span><i aria-hidden="true">{pictureFor(o,stage,i)}</i><b>{o}</b></button>)}</div>{checked&&<div className={choice===q.answer?"feedback success":"feedback try"}><strong>{choice===q.answer?"Fin-tastic!":"Good try!"}</strong><p>{q.explanation}</p></div>}<button className="primary-button compact" disabled={choice===null} onClick={checked?next:choose}>{checked?(pos===4?"Finish quiz":"Next question →"):"Check answer"}</button></article>
}
